...
public:
  ...
  vector<vector<DropoutNode>> _dropout_nodes_after_hidden_nodes;
...
public:
  inline void initial(Graph *pcg, ModelParams &model, HyperParams &opts,
    ...
    for (int idx = 0; idx < _word_inputs.size(); idx++) {
      ...
      _dropout_nodes_after_input_nodes[idx].init(opts.wordDim);
      _dropout_nodes_after_input_nodes[idx].setParam(0.0);
      ...
      for (int idy = 0; idy < cnnLayerSize; idy++) {
        ...
        _dropout_nodes_after_hidden_nodes[idy][idx].init(opts.hiddenSize);
        _dropout_nodes_after_hidden_nodes[idy][idx].setParam(0.0);
      }
    }
...
public:
  ...
  inline void forward(const Feature &feature, bool bTrain = false) {
    ...
    _word_window_layers[0].forward(_pcg,
    ...
    for (int i : boost::irange(0, words_num)) {
      _dropout_nodes_after_hidden_nodes[0][i].forward(_pcg,
          &_hidden_layers[0][i]);
    }

    int cnnLayerSize = _word_window_layers.size();
    for (int i = 1; i < cnnLayerSize; i++) {
      _word_window_layers[i].forward(_pcg,
          getPNodes(_dropout_nodes_after_hidden_nodes[i - 1], words_num));

      for (int j = 0; j < words_num; j++) {
        _hidden_layers[i][j].forward(_pcg,
            &_word_window_layers[i]._outputs[j]);
        _dropout_nodes_after_hidden_nodes[i][j].forward(_pcg,
            &_hidden_layers[i][j]);
      }
    }

    _avg_pooling.forward(_pcg,
        getPNodes(_dropout_nodes_after_hidden_nodes[cnnLayerSize - 1], words_num));
